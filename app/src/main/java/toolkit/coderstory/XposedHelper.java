package toolkit.coderstory;

import com.coderstory.toolkit.BuildConfig;

import java.util.Set;

import de.robv.android.xposed.XC_MethodHook;
import de.robv.android.xposed.XSharedPreferences;
import de.robv.android.xposed.XposedBridge;
import de.robv.android.xposed.XposedHelpers;


public class XposedHelper {
    public String SIGNATURE = "";

    public static XSharedPreferences PREFS = new XSharedPreferences(BuildConfig.APPLICATION_ID, "conf");

    /**
     * 生成一个修改返回值的hook方法
     *
     * @param confKey         配置key, 读取不到值时默认为true
     * @param targetReturnVal 如果配置为true时返回的数据
     * @return 修改返回值的hook方法
     */
    public static XC_MethodHook returnValChanger(String confKey, Object targetReturnVal) {
        return returnValChanger(confKey, true, targetReturnVal);
    }
    /**
     * 生成一个修改返回值的hook方法
     *
     * @param confKey         配置key
     * @param confValDefault  配置读取不到值时默认value
     * @param targetReturnVal 如果配置为true时返回的数据
     * @return 修改返回值的hook方法
     */
    public static XC_MethodHook returnValChanger(String confKey, boolean confValDefault, Object targetReturnVal) {
        return new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                // 指定配置项为true则修改返回值为指定值
                if (getConfVal(confKey, confValDefault)) param.setResult(targetReturnVal);
            }
        };
    }

    /**
     * 重新载入配置后取出指定配置值
     *
     * @param confKey 配置key
     * @return 对应的配置值, 默认为true
     */
    public static boolean getConfVal(String confKey) {
        // 默认返回 true
        return getConfVal(confKey, true);
    }
    /**
     * 重新载入配置后取出指定配置值
     *
     * @param confKey 配置key
     * @param defaultVal 未读取到配置时的默认返回值
     * @return 对应的配置值
     */
    public static boolean getConfVal(String confKey, boolean defaultVal) {
        try {
            PREFS.reload();
            return PREFS.getBoolean(confKey, defaultVal);
        } catch (Throwable e) {
            XposedBridge.log(e);
            return defaultVal;
        }
    }

    public static void findAndHookMethod(String p1, ClassLoader lpparam, String p2, Object... parameterTypesAndCallback) {
        try {
            if (findClass(p1, lpparam) != null) {
                XposedHelpers.findAndHookMethod(p1, lpparam, p2, parameterTypesAndCallback);
            }
        } catch (Throwable e) {
            XposedBridge.log(e);
        }
    }

    public static void hookAllMethods(String p1, ClassLoader lpparam, String methodName, XC_MethodHook parameterTypesAndCallback) {
        try {
            Class<?> packageParser = findClass(p1, lpparam);
            XposedBridge.hookAllMethods(packageParser, methodName, parameterTypesAndCallback);
        } catch (Throwable e) {
            XposedBridge.log(e);
        }

    }

    public void hookAllMethods(Class<?> packageManagerServiceUtils, String verifySignatures, XC_MethodHook methodHook) {
        try {
            XposedBridge.hookAllMethods(packageManagerServiceUtils, verifySignatures, methodHook);
        } catch (Throwable e) {
            XposedBridge.log(e);
        }
    }

    public static void hookAllConstructors(String p1, XC_MethodHook parameterTypesAndCallback) {
        try {
            Class<?> packageParser = findClass(p1, null);
            hookAllConstructors(packageParser, parameterTypesAndCallback);
        } catch (Throwable e) {
            XposedBridge.log(e);
        }
    }

    private static Set<XC_MethodHook.Unhook> hookAllConstructors(Class<?> hookClass, XC_MethodHook callback) {
        try {
            return XposedBridge.hookAllConstructors(hookClass, callback);
        } catch (Throwable e) {
            XposedBridge.log(e);
            return null;
        }
    }

    public static Class<?> findClass(String className, ClassLoader classLoader) {
        try {
            return Class.forName(className, false, classLoader);
        } catch (Throwable e) {
            XposedBridge.log(e);
        }
        return null;
    }
}
